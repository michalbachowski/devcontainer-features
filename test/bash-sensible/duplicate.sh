...
check "The $RC_FILE_NAME [$RC_FILE_PATH] file contains ONLY ONE reference to the $SENSIBLE_FILE_NAME [$SENSIBLE_FILE_PATH] file" bash -c "test \$(sudo cat $RC_FILE_PATH | grep 'source $PROMOPT_FILE_PATH' | grep source | wc -l) -eq '1'"
