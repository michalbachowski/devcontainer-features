...
check "The $RC_FILE_NAME [$RC_FILE_PATH] file contains a reference to the $PROMPT_FILE_NAME [$PROMPT_FILE_PATH] file" bash -c "sudo cat $RC_FILE_PATH | grep '$PROMPT_FILE_NAME' | grep -q source"
...
check "When sourcing the [$USER_RC_FILE_PATH] script file, the prompt takes effect" zsh -c "source $USER_RC_FILE_PATH; echo \$RPS1 | grep -q reset_color"
